function agregar() {
    ...
    $.post("../../../clases/ultimo_codigo.php", {pk: "id_cargo", tabla: "cargos"})
            .done(function (data) {
                $("#txtcodigo").val(data);
            });
}
...
function grabar() {
    ...
    if (d === "") {
        ...
        var sql = "select sp_cargos("+$("#txtcodigo").val()+",'"+$("#txtdescripcion").val()+"',"+$("#operacion").val()+")";
        ...
        if(oper === "2"){
            conf = "¿DESEA ACTUALIZAR ESTE CARGO?";
        }
        if(oper === "3"){
