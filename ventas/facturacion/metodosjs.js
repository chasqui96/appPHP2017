...
$(document).ready(function () {
    ...
    $('#clienteFisico').change(function() {
        // Si el checkbox de cliente físico se marca, desmarca el checkbox de cliente jurídico
        if ($(this).is(':checked')) {
            $('#clienteJuridico').prop('checked', false);
        }
    });
    
    $('#clienteJuridico').change(function() {
        // Si el checkbox de cliente jurídico se marca, desmarca el checkbox de cliente físico
        if ($(this).is(':checked')) {
    
            $('#clienteFisico').prop('checked', false);
        }
    });
    $('#clienteFisico').prop('checked', true);
    $("#id_ciudad").val(12).trigger("chosen:updated");
});
...
$('#ruc_numero').keypress(function (e) {
    if (e.which === 13) {
      var dv =  calcularDigitoVerificador($('#ruc_numero').val(),11);
      $('#dv').val(dv);
    }
...
$('#agregarClienteBtn').click(function(event) {
    ...
    $.ajax({
      ...
      success: function(response) {
        if(response > 0){
            ...
            bootbox.alert("El Cliente fue Agregado Con existo!");
            limpiarFormulario();
        }else{
